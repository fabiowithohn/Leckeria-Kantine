...
// Fester Schlüssel für den wöchentlichen Grenzebach-Speiseplan (als PDF).
export const GRENZEBACH_PLAN_ID = "grenzebach-plan";

/** Metadaten des Grenzebach-PDF-Plans (ohne die Dateidaten zu laden). */
export async function getGrenzebachPlanMeta(): Promise<{
  hasFile: boolean;
  title: string | null;
  version: string;
} | null> {
  ...
  const filed = await prisma.weeklyPlan.findFirst({
    where: { id: GRENZEBACH_PLAN_ID, NOT: { imageData: null } },
    select: { id: true },
  });
  if (!plan) return null;
  return {
